...
def cnn_encoder(filter_sizes, embedding_size, sequence_length, embedded_chars_expanded, dropout_keep_prob, num_filters):
    ...
    for i, filter_size in enumerate(filter_sizes):
        ...
        avg_pooled_outputs.append(avg_pooled)

        # Combine all the pooled features
    num_filters_total = num_filters * len(filter_sizes)
    h_pool = tf.concat(pooled_outputs, 3)
    h_pool_flat = tf.reshape(h_pool, [-1, num_filters_total])
    h_avg_pooled = tf.concat(avg_pooled_outputs, 3)
    h_avg_flat = tf.reshape(h_avg_pooled, [-1, num_filters_total])
    h_out = tf.concat([h_pool_flat, h_avg_flat], 1)
    print("h pool flat", h_pool_flat)
    print("h_avg_flat", h_avg_flat)
    print("h_out", h_out)
        # Add dropout

    h_drop = tf.nn.dropout(h_out, dropout_keep_prob)
